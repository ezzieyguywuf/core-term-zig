...
const RenderActor = struct {
    ...
    ansi_parser: ansi_parser_mod.AnsiParser,
            ...
            pub fn init(allocator: std.mem.Allocator) !RenderActor {
                ...
                std.debug.print("client.width type: {s}, value: {d}\n", .{ @typeName(@TypeOf(client.width)), client.width });
                std.debug.print("client.height type: {s}, value: {d}\n", .{ @typeName(@TypeOf(client.height)), client.height });
        
                const client_width_f32 = @as(f32, client.width);
                const client_height_f32 = @as(f32, client.height);
