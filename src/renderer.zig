        ...
        for (0..terminal_grid.width) |col_idx| {
            ...
            const cell_painter = struct {
                pub fn eval(c: CellRenderContext, x_p: pf.Field, y_p: pf.Field) struct { r: pf.Field, g: pf.Field, b: pf.Field, a: pf.Field } {
                    ...
                    var final_r = bg_r_field;
                    var final_g = bg_g_field;
                    var final_b = bg_b_field;

                    // Convert global pixel coordinates to local character cell coordinates
                    const local_x = x_p - pf.Core.constant(c.cell_pixel_x);
                    const local_y = y_p - pf.Core.constant(c.cell_pixel_y);
                    ...
                    for (0..pf.LANES) |lane_idx| {
                        ...
                        if (lx_scalar >= 0 and lx_scalar < CHAR_WIDTH and ly_scalar >= 0 and ly_scalar < CHAR_HEIGHT) {
                            ...
                            if (font_row_idx < font.FONT_HEIGHT and font_col_idx < font.FONT_WIDTH) {
                                ...
                                if (is_pixel_set != 0) {
                                    final_r.set(lane_idx, fg_r_field.get(lane_idx));
                                    final_g.set(lane_idx, fg_g_field.get(lane_idx));
                                    final_b.set(lane_idx, fg_b_field.get(lane_idx));
                                    
                                    if (c.is_bold) {
                                        final_r.set(lane_idx, @min(1.0, fg_r_field.get(lane_idx) * 1.2));
                                        final_g.set(lane_idx, @min(1.0, fg_g_field.get(lane_idx) * 1.2));
                                        final_b.set(lane_idx, @min(1.0, fg_b_field.get(lane_idx) * 1.2));
                                    }
                                }
                            }
                        }
